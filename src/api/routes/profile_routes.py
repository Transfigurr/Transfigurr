...
@router.get("/api/profiles", tags=["Profiles"], name="Get All Profiles")
async def get_all_profiles_route(user: str = Depends(login_with_token)):
    return await get_all_profiles()


@router.get("/api/profiles/{profile_id}", tags=["Profiles"], name="Get Profile By Id")
...
@router.put('/api/profiles', tags=["Profiles"], name="Upsert Profile")
...
@router.delete('/api/profiles/{profile_id}', tags=["Profiles"], name="Delete Profile By Id")
